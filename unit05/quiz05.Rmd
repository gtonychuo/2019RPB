...
以下這一段簡單的程式碼就可以畫出數值與類別變數的數量與機率分佈；你可以在雙引號之間補進正確的標題，如："類別變數的數量分布"、"數量"、"機率"、"密度"等等，讓圖形更加完整，更能幫助同學們分辨這四種不同的分佈嗎？ 程式筆記在這裡，請將妳做好的圖形直接貼在這則po文下面，最先做出正確答案的同學可以得到精美獎品一份(NT$300禮卷)，如果妳覺得圖形還有可以改善的空間，就可以繼續po，只要我們覺得妳的改進有道理，妳也可以得到相同獎品。 
</p class="wwl">
