...
par(cex=0.8)
options(scipen=20, digits=5, width=90)
library(magrittr)
```
<hr>

**單元摘要：**使用R複習並練習操作「機率」的基本觀念
...
* 分布 Distribution 
    ...
    + 實證/理論分布: Emperical/Theoretical Distribution
...
barplot(table(Skin), main="Dist. of Skin Color",xlab="Skin Color", ylab="Count")
```
...
par0 = par(cex=0.3, mfrow=c(1,2))
hist(Weight)
barplot(table(Skin), main="Dist. of Skin Color",xlab="Skin Color", ylab="Count",las=2)
```

<span style="font-size:24px">`r emo::ji("moai")`:</span> 
*直條圖和直方圖有什麼異同？* <br><br><hr>
...
<br><br>
<span style="font-size:24px">`r emo::ji("moai")`:</span> 
*`binwidth`代表什麼？ 它會如何改變直方圖的形狀？ 為甚麼？*<br>
...
<p class="wwl">
<span style="font-size:24px">`r emo::ji("light_bulb")` 學習重點：</span><br>
...
<p class="qiz">
<span style="font-size:24px">`r emo::ji("moai")` 問題討論：</span><br>
&nbsp; ■ &nbsp; 直方圖和數量分布函數之間有何異同？<br>
&nbsp; ■ &nbsp; 數量分布函數和機率函數之間呢？<br>
&nbsp; ■ &nbsp; 機率函數下方的總面積分是多大呢？<br>
</p class="qiz"><br>
...
Trials = list(
  ...
  skin1_1M = replicate(1000000, sample(Skin, size=1))
  )
Sys.time() - t0
```
...
for(v in Trials) { hist(
  v, breaks=seq(40,80,2.5), 
  main=paste(length(v),"repeats"), 
  xlab="Weight"
  )}
...
```{r fig.height=4}
lapply(10^(1:6), replicate, sample(Weight, 1)) %>% lapply(function(v) {
